...
Do not hardcode component radii (`16px`, `24px`, `999px`, etc.) for buttons, chips, cards, dialogs, scroll thumbs, inputs, or repeated rows. Use `--brand-radius-sm/md/lg/pill` so presets such as Apple, Carbon, Linear, and Terminal can change the UI shape globally. Ordinary toolbar buttons and selectable chips should use `--brand-radius-md`; reserve `--brand-radius-pill` for badges, avatars, circular icon controls, and genuinely pill-shaped status affordances. Fixed pixel radii are acceptable only for asset-internal geometry where the shape is not a UI component contract.
...
When using `UTextarea` for a tool editor, remember that `class` lands on Nuxt UI's root wrapper, not the actual `<textarea>`. Put height, padding, font, background, and placeholder rules on an internal class via `:ui="{ base: '...' }"` and style it with `:deep(.your-editor-base)`. Otherwise the visible input can collapse to Nuxt UI's default 3-row textarea even when the wrapper has a large `min-height`. Tool editors should use brand/system theme tokens for background, text, border, caret, and placeholder colors; add `:global(html.dark)` / dark-brand overrides only where the theme needs a different surface.

For rendered Mermaid previews, configure Mermaid from live brand tokens (`--brand-color-*`, `--brand-font-family`) and re-render when `html.dark` or `html[data-brand-theme]` changes. Do not hardcode light Mermaid colors inside the render config; otherwise the SVG can mix black labels, white labels, and stale node fills after a system theme switch. Export actions such as `复制源码` / `下载 SVG` belong in the `ToolWorkbench` toolbar slot with save/history/example controls, not inside the source editor pane.
...
Nuxt UI runs inside the existing Quasar/Vite shell, not Nuxt. Keep `colorMode: false` in `quasar.config.js`; the project owns theme switching in `SiteThemeSwitcher.vue`. Color mode toggles `html.dark` and stores `xinming-color-mode`. Brand presets store `xinming.brand.theme`, apply `html[data-brand-theme]` through `src/boot/brand-theme.js`, and must remain available when replacing controls (`editorial`, `terminal`, `apple`, `carbon`, `fluent`, `primer`, `stripe`, `linear`, `material`, `spotify`, `claude`, `vercel`). Map Nuxt UI color and radius variables back to project brand tokens in `src/css/app.scss` so `primary`, `neutral`, `text-inverted`, and generated `rounded-*` classes stay legible and on-brand in dark and branded modes. Use the global `brand-action-button` class on solid primary call-to-action `UButton`s that should follow brand accent, contrast, and `--brand-action-radius` instead of Nuxt UI's default `rounded-md`. Do not use `UColorModeSelect` or `UApp` unless the app shell is changed and browser-verified again. Shell navigation uses `UButton` links plus `UDropdownMenu`; after changing shell menus, browser-verify the app for Vue recursive render errors before finishing.

Reka UI is not a direct dependency. Do not add direct `reka-ui` imports. Keep direct native controls rare and justified; unavoidable browser primitives such as a hidden file input may be wrapped by a Nuxt UI trigger. For custom upload triggers, prefer an actual hidden native `<input type="file">` inside a labeled `UButton`; do not use a nested `UInput type="file"` inside `UButton`, because Nuxt UI's input root can consume the button layout and hide the trigger text. Vue Router's dynamic `<component :is="Component">` in `MainLayout.vue` remains the route outlet rather than a UI control.
