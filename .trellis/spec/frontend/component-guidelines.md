...
Nuxt UI runs inside the existing Quasar/Vite shell, not Nuxt. Keep `colorMode: false` in `quasar.config.js`; the project owns theme switching in `SiteThemeSwitcher.vue`. Color mode toggles `html.dark` and stores `xinming-color-mode`. Brand presets store `xinming.brand.theme`, apply `html[data-brand-theme]` through `src/boot/brand-theme.js`, and must remain available when replacing controls (`editorial`, `terminal`, `apple`, `carbon`, `fluent`, `primer`, `stripe`, `linear`, `material`, `spotify`, `claude`, `vercel`). Map Nuxt UI color variables back to project brand tokens in `src/css/app.scss` so `primary`, `neutral`, and `text-inverted` stay legible in dark and branded modes. Do not use `UColorModeSelect` or `UApp` unless the app shell is changed and browser-verified again. Shell navigation uses `UButton` links plus `UDropdownMenu`; after changing shell menus, browser-verify the app for Vue recursive render errors before finishing.
