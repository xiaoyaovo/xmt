...
When using `UTextarea` for a tool editor, remember that `class` lands on Nuxt UI's root wrapper, not the actual `<textarea>`. Put height, padding, font, background, and placeholder rules on an internal class via `:ui="{ base: '...' }"` and style it with `:deep(.your-editor-base)`. Otherwise the visible input can collapse to Nuxt UI's default 3-row textarea even when the wrapper has a large `min-height`.
